...
parser.add_argument('-r', '--betarun', dest='betarun', required=False, type=int)
parser.add_argument('-cL', '--cfdLGAD', dest='cfdLGAD', required=True, type=str)
parser.add_argument('-c', '--cfdPhotek', dest='cfdPhotek', required=True, type=str)
...
print opt.min
print opt.max
print "photek cfd: ", min_photek_cfd
...
if sensor == 'Beta':
    runs = [[betarun]]
if sensor == 'Pre-rad_UCSC_195':
    runs = runs_4
if sensor == '5E14_UCSC_500':
    runs = runs_5
if sensor == '5E14_UCSC_540':
    runs = runs_6
...
for f in freq:
    ...
    for run in runs:
        for r in run:
            ...
            if (f==2500):
               ch.Add('root://cmseos.fnal.gov//store/user/twamorka/FilterStudy/UnfilteredFiles/run_scope'+str(r)+'.root')
            else: ch.Add('root://cmseos.fnal.gov//store/user/twamorka/FilterStudy/dat2Root_Filter/run_scope_dat2root_'+str(r)+'_'+str(f)+'.root')
    ...
    # min_photek_cfd = 0
    min_lgad_cfd = 0
    # #--loop over photek cfd values to determine photek cfd corresponding to min. time resolution
    # for c in cfd:
    ...
    # min_cfdPhotek.append(min_photek_cfd)
# print min_cfdPhotek
#-- loop over lgad cfd; photek cfd is fixed. For each filter frequency, the lgad cfd corresponding to min. time resolution is determined.
    for c in cfd:
        ...
        c0 = TCanvas('c0','c0',800,600)
        h_lgad.Draw()
        c0.SaveAs(outputLoc+'freq_'+str(f)+'_lgadcfd'+'_LGADcfd_'+str(c)+'.pdf')
        c0.SaveAs(outputLoc+'freq_'+str(f)+'_lgadcfd'+'_LGADcfd_'+str(c)+'.png')
    ...
    npoints = len(cfd_arr)
    MakeGraphErrors(npoints,array('d',cfd_arr),array('d',timeLGAD_res),array('d',cfd_err_arr),array('d',timeLGAD_res_err),'LGAD CFD','Time Resolution [ps]',outputLoc,'freq_'+str(f)+'_vs_LGADcfd'+str(sensor))
...
#     ch.Draw('LP2_'+str(min_lgad_cfd)+'['+str(Ch)+'] - LP2_'+str(min_photek_cfd)+'[3] >>' +hname,TCut('LP2_'+str(min_lgad_cfd)+'['+str(Ch)+']!=0&&LP2_'+str(min_photek_cfd)+'[3]!=0&&amp['+str(Ch)+']>'+str(ampCut)+'&&amp[3]>'+str(photekMinCut)+'&&amp[3]<'+str(photekMaxCut)))
...
#     slewrate_arr_ps.append(abs(h3.GetMean()/1e3))
#
#     print "Slewrate array :", slewrate_arr
#
#     h4 = TH1F(hname,hname,100,0,200)
#     # h4 = TH1F(hname,hname,50,40,300)
...
# jitter = [float(ai)/bi for ai,bi in zip(baselineRMS_arr,slewrate_arr_ps)]
#
...
# MakeGraphErrors(nfreq,freq_arr,slewrate_arr,freq_err_arr,slewrate_err_arr,'Filter Frequency [MHz]','Slew Rate (dV/dt) [mV/ns]',outputLoc,'dVdT_vs_Freq_'+str(sensor))
...
# MakeGraph(nfreq,freq_arr,min_cfdPhotek,'Filter Frequency [MHz]','Min. Photek cfd',outputLoc,'MinPhotek_vs_Freq_'+str(sensor))
